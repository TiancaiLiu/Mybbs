...
if(isset($_POST['submit'])) {
	...
	";
	$query['sfc_info']="
		CREATE TABLE IF NOT EXISTS `sfc_info` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
		  `title` varchar(255) NOT NULL,
	...
	$query_info_s="SELECT * from `sfc_info` where id=1";
	$result=mysqli_query($link, $query_info_s);
	if(mysqli_num_rows($result)!=1){
		$query_info_i="INSERT INTO `sfc_info` (`id`, `title`, `keywords`, `description`) VALUES(1, 'sfkbbs', '私房菜', '私房菜');";
		mysqli_query($link, $query_info_i);
		if(mysqli_errno($link)){
			exit("数据库sfc_info写入数据失败请检查相应权限!<a href='install.php'>点击返回</a>");
		}
	}
	$query_manage_s="SELECT * from `sfc_manage` where name='admin'";
	$result=mysqli_query($link, $query_manage_s);
	if(mysqli_num_rows($result)!=1){
		$query_manage_i="INSERT INTO `sfc_manage` (`name`, `pw`, `create_time`, `level`) VALUES('admin',md5('{$_POST['manage_pw']}'),now(), 0)";
