<?php 
	...
	if(is_manage_login($link)) {
		skip('2', 'index.php', 'error', '您已经登录，请不要重复登录！'); 
	}
	if(isset($_POST['submit'])) {
		include 'inc/check_login.inc.php';
		$_POST = escape($link, $_POST);
		$query = "SELECT * from `sfc_manage` where name='{$_POST['name']}' and pw=md5('{$_POST['pw']}')";
		...
		if(mysqli_num_rows($result)==1) {
			...
			skip('2', 'index.php', 'ok', '恭喜你，登陆成功！'); 
		}else{
			skip('2', 'login.php', 'error', '登录失败，请检查用户名或密码输入是否有误！'); 
		}
}
?>
<!DOCTYPE html>
